...
typedef struct Env { 
  FILE * file;
  const char * prefix;
  int round, bugs, violations, seed, print, quiet, first, nodd, noptsfuzz;
...
static void printrace (Env * env) {
  ...
  sprintf (name, "%s-%d.trace", env->prefix, env->seed);
...
static void prwc (Env * env, const char * prefix) {
  ...
  int ch, res = 0;
  FILE * file;
  sprintf (name, "%s-%d.trace", prefix, env->seed);
  file = fopen (name, "r");
  assert (file);//FIXME
...
static void dd (Env * env, const char * filename, int golden, int opt) {
  ...
  while (fscanf (file, "%p %x\n", &state, &rand) == 2)
    env->nevents += 1;
  fclose (file);
  env->events = calloc (env->nevents, sizeof *env->events);
  file = fopen (filename, "r");
  assert (file);//FIXME
  i = 0;
  while (fscanf (file, "%p %x\n", &state, &rand) == 2) {
  ...
  if (env->nodd)
    sprintf (cmd, 
	     "cp bug-%d.trace red-%d.trace",
	     env->seed, env->seed);
  else
    sprintf (cmd, 
	     "./lglddtrace %s bug-%d.trace red-%d.trace",
