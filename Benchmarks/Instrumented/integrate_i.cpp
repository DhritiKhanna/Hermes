...
double worker(double a, double f(double)) {
  ...
  while (1) {
    ...
    if (status.MPI_TAG == WORK_TAG) {
      ...
      ifs->storeInfo("status.MPI_TAG",bo_eq,1,93,97,92,97); MPI_Send(&result, 1, MPI_DOUBLE, 0, RESULT_TAG, MPI_COMM_WORLD);
    } else {
      ifs->storeInfo("status.MPI_TAG",bo_eq,status.MPI_TAG,98,99,92,-1); break;
    }
  }
  fflush(stdout);
  return 0.0;
