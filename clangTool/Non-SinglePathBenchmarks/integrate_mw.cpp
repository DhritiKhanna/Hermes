...
double worker(double a, double f(double), int rank) {
...
double integral(double a, double b, double tolerance, double f(double), int rank) {
...
int main(int argc, char *argv[]) {
  ...
  MPI_Init(&argc, &argv);
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  result = integral(0, pi, 1, f1, rank);
