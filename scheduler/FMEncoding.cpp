...
void FMEncoding::createEventLiterals ()
{
  ...
  forall_transitionLists(iter, last_node->_tlist){
    forall_transitions(titer, (*iter)->_tlist){
      ...
      if(env->func_id == FINALIZE || env->func_id == PROBE || env->func_id == IPROBE) continue; // dhriti: Do not create event literals for PROBE and IPROBE also.
...
void FMEncoding::createMatchLiterals()
{
  ...
  forall_matchSet(mit, matchSet){
    forall_match(lit, (**mit)){
	 ...
	 last_node->GetTransition(*lit)->GetEnvelope()->isRecvTypeOnly()){ // dhriti: isRecvTypeOnly instead of isRecvType
	uniquepair<<(*lit)._pid;
...
void FMEncoding::createRFSomeConstraint()
{
  ...
  forall_transitionLists(iter, last_node->_tlist){
    forall_transitions(titer, (*iter)->_tlist){
      ...
      if(envA->isRecvTypeOnly() || envA->isSendType()) { // dhriti: isRecvTypeOnly instead of isRecvType
	forall_matchSet(mit, matchSet){
	  forall_match(lit, (**mit)){
	    if((*lit) == A) {
	      flag = true; 
...
void FMEncoding::uniqueMatchSend()
{
  ...
  forall_matchSet(mit, matchSet){
	...
	if(recv != (*sit)){
	  ...
	  formula << "(" << getLitName(s_m, 0) << " -> !" << getLitName(s_n, 0) << ") &" <<std::endl;
...
void FMEncoding::matchImpliesIssued()
{
  ...
  forall_matchSet(mit, matchSet){
    ...
    if(front->isCollectiveType()){
      literalt m_match;
      bvt i_evts;
      std::map<CB, std::pair<literalt, literalt> >::iterator eit = eventMap.find(A);
      if(eit != eventMap.end()){
