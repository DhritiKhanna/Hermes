...
#define nprocsy 2   /* number of processes, y direction           */ // Make it 4 for -n 8
...
#define nylg   4   /* nyl + 2 (ghost cells)                      */ // Make it 6 for -n 8
...
int np, rank, xcoord, ycoord, up, down, lef, righ;
double u[nxlg][nylg];
...
void initdata() { 
  ...
  lef = (xcoord + nprocsx - 1)%nprocsx + nprocsx*ycoord;
  righ = (xcoord + 1)%nprocsx + nprocsx*ycoord;
  for (i = 1; i <= nxl; i++) 
    for (j = 1; j <= nyl; j++) { 
...
void exchange_ghost_cells() {
  ...
  /* MPI_Sendrecv(sbufy, nyl, MPI_DOUBLE, lef, 0, rbufy, nyl, */
  /*              MPI_DOUBLE, righ, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
  MPI_Send(sbufy, nyl, MPI_DOUBLE, lef, 0, MPI_COMM_WORLD);
  MPI_Recv(rbufy, nyl, MPI_DOUBLE, righ, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  
  for (j = 1; j <= nyl; ++j) u[nxl+1][j] = rbufy[j-1];

  for (j = 1; j <= nyl; ++j) sbufy[j-1] = u[nxl][j];
  /* MPI_Sendrecv(sbufy, nyl, MPI_DOUBLE, righ, 0, rbufy, nyl, */
  /*              MPI_DOUBLE, lef, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
  MPI_Send(sbufy, nyl, MPI_DOUBLE, righ, 0, MPI_COMM_WORLD);
  MPI_Recv(rbufy, nyl, MPI_DOUBLE, lef, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
