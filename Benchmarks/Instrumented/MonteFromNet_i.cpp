    ...
    {	/* I am the rand server */
		do 
		{
		    ...
		    if (request == 1) 
		    {
				...
				ifs->storeInfo("request",bo_eq,1,49,57,48,57); MPI_Send(rands, CHUNKSIZE, MPI_INT, status.MPI_SOURCE, REPLY, world);
			} else { ifs->storeInfo("request",bo_eq,request,58,58,48,-1);  }
		} while(request > 0);
    }
    else 
