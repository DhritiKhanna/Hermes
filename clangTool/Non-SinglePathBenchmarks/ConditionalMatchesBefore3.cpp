...
int main(int argc, char** argv) 
{
  ...
  else if (world_rank == 1)
  {
    ...
    token2 = 11;
    token3 = 12;
    MPI_Send(&token1, 1, MPI_INT, 4, 10, MPI_COMM_WORLD);
    MPI_Send(&token2, 1, MPI_INT, 0, 11, MPI_COMM_WORLD);
    MPI_Send(&token3, 1, MPI_INT, 3, 12, MPI_COMM_WORLD);
  }
  ...
  else if (world_rank == 3)
  {
    ...
    MPI_Recv(&token2, 1, MPI_INT, 2, MPI_ANY_TAG, MPI_COMM_WORLD, &status2);
  }
