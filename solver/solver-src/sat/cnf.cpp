...
literalt cnft::land(const bvt &bv)
{
  ...
  forall_literals(it, bv){
    constraintStream << "l_" << (*it).get() << " & "; 
  }
  constraintStream <<std::endl;
...
{
  // svs
  constraintStream << "l_" << a.get() << " & " << "l_" << b.get()<< std::endl;
...
literalt cnft::lnot(literalt a)
{
  a.invert();
  //svs 
  constraintStream << " ! l_" << a.get() << std::endl;
  return a;
}
...
// literalt cnft::unsigned_less_than(
//   const bvt &op0,
//   const bvt &op1)
// {
//   // A <= B  iff  there is an overflow on A-B

//   return lnot(carry_out(op0, inverted(op1), const_literal(true)));
// }

// literalt cnft::carry_out(
//   const bvt &op0,
//   const bvt &op1,
//   literalt carry_in)
// {
...
// bvt cnft::inverted(const bvt &bv)
// {
//   bvt result=bv;
...
// literalt cnft::carry(literalt a, literalt b, literalt c)
// {
//   bvt tmp;
...
// literalt cnft::equal(const bvt &op0, const bvt &op1)
// {
//   assert(op0.size()==op1.size());
...
//   for(unsigned i=0; i<op0.size(); i++)
//     equal_bv[i]=lequal(op0[i], op1[i]);

//   return land(equal_bv);
// }

// literalt cnft::is_zero(const bvt &op)
// {
//   return lnot(lor(op)); 
// }
