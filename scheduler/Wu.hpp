...
public:
  ...
  */
  ...
  void AddWESet(std::set<int> cfrom, int pid, CB to) ;
  /*
  ...
  */
  ...
  bool ApplyRefineRuleOne(Node *, int&);
  bool ApplyRefineRuleTwo(Node *, int &);

  //void DeadlockDetection(std::vector<std::list <CB> > al, Node *);
  
  bool DeadlockDetection(std::vector<std::list <CB> > al, Node *last, int &spid);
  ...
  void RemoveMatchedSends(std::set<CB> &res, std::vector<std::list<CB> > l, CB);

  void RemoveWorderedSends(std::set<CB> &res, CB s, Node *n);

  CB isSendTrgtofWait(CB s, CB r, Node *n);
