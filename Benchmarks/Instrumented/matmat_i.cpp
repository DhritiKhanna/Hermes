...
int main(int argc, char **argv) 
{
  ...
  else 
  {
    ...
    while (1) 
    {
      ...
      if (status.MPI_TAG == 0) 
      {
        ifs->storeInfo("status.MPI_TAG",bo_eq,0,122,124,121,-1); break;
      }
      else
      {
        //vecmat(in, b, out);
        ...
        for (j = 0; j < M; j++)
          for (k = 0, out[j] = 0; k < L; k++)
            out[j] += in[k]*b[k][j];
        ifs->storeInfo("status.MPI_TAG",bo_eq,status.MPI_TAG,127,133,121,133); MPI_Send(out, M, MPI_INT, 0, status.MPI_TAG, comm);
      }
    }
  }
